...
; text => (test-b (test (op =) (reg b) (const 0))
...
; ......
; next-inst => test-b
; insts => (((test (op =) (reg b) (const 0)))
;	    ((branch (label gcd-done)))
;	    ((assign t (op rem) (reg a) (reg b)))
;	    ((assign a (reg b)))
;	    ((assign b (reg t)))
;	    ((goto (label test-b))))
; labels => ((test-b ((test (op =) (reg b) (const 0)))
;	                ((branch (label gcd-done)))
;	                ((assign t (op rem) (reg a) (reg b)))
;	                ((assign a (reg b)))
;	                ((assign b (reg t)))
...
(define (instruction-text inst)
  ; inst => ((assign t (op rem) (reg a) (reg b)) . #<procedure:...5/ch5-regsim.scm:272:6>)
  (car inst))
...
(define (set-instruction-execution-proc! inst proc)
  ; inst => (assign t (op rem) (reg a) (reg b))
  ; proc => #<procedure:...5/ch5-regsim.scm:272:6>)
  (set-cdr! inst proc))
  ; inst => ((assign t (op rem) (reg a) (reg b)) . #<procedure:...5/ch5-regsim.scm:272:6>)
...
(define (assign-reg-name assign-instruction)
  ; assign-instruction => (assign t (op rem) (reg a) (reg b))
...
(define (make-branch inst machine labels flag pc)
  ...
  ;(dbg labels)
  ;(dbg flag)
  ; inst => (branch (label gcd-done))
  (let ((dest (branch-dest inst)))
    ; dest => (label gcd-done)
    (if (label-exp? dest)
        (let ((insts
               (lookup-label labels (label-exp-label dest))))
          ;  (label-exp-label dest) => gcd-done
          (lambda ()
